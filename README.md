...
Set an `*_APPLY` option to `0` if you want dry-run mode.

## Generated Files

- `last_sync_state` - shared sync state
- `logs/task_sync_<device>.log` - per-device log
- `.task_sync.lock` - local lock directory while the script is running

## Notes

- This does not configure Taskwarrior sync for you. Make sure `task sync` works
  first.
- If the network check is a problem on your setup, set
